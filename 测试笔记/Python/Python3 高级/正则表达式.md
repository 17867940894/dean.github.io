...
```python
import re
res = re.match('www', 'www.runoob.com')
print(res.group())
```

把**正则匹配到的那一段文字**拿出来，返回字符串。

上面输出：`www`

#### **不带参数 group () /group (0)**: `group()` 和 `group(0)` 效果完全一样，代表**整体匹配到的全部内容**

#### 带括号分组 group (1)、group (2)

正则里用 `()` 划分小组，就能用数字取出每组内容

```python
import re
text = "apple123"
# 两个括号，分成两组
obj = re.match(r'(apple)(\d+)', text)

print(obj.group(0))  # 全部匹配：apple123
print(obj.group(1))  # 第1组：apple
print(obj.group(2))  # 第2组：123
```

#### 一次性取多组 groups ()

```python
print(obj.groups())  # ('apple', '123')
```

**如果匹配失败，res 是 `None`，调用 `.group()` 直接报错**
...
语法格式为：`re.findall(pattern, string, flags=0)`  或  `pattern.findall(string[, pos[, endpos]])`
